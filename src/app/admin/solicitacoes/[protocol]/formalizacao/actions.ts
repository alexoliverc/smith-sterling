...
const disbursementSchema = z.object({
  ...
  reference: z
    .string()
    .trim()
    .min(4, 'Informe a referência da transferência.')
...
export async function confirmFormalizationReady(
  ...
  revalidateFormalizationPaths(parsed.data);
...
export async function registerDisbursement(
  ...
  /*
   * Não confiamos apenas no atributo
   * required do checkbox no navegador.
   *
   * A confirmação é validada novamente
  ...
  if (!confirmed) {
    return {
      error: 'Confirme que a transferência já foi efetivamente realizada.',
    };
  }
  ...
  if (!parsed.success) {
    return {
      error: parsed.error.issues[0]?.message ?? 'Dados da liberação inválidos.',
    };
  }
  ...
  if (session.user.role !== 'SUPER_ADMIN') {
    return {
      error: 'Seu perfil não possui autorização para registrar a liberação.',
    };
  }

  const application = await findAdminFormalizationForTransition(parsed.data.protocol);
  ...
  if (application.formalization.status !== 'READY_FOR_DISBURSEMENT') {
    return {
      error: 'Esta operação ainda não está pronta para registrar a liberação.',
    };
  }

  const protectedReference = encryptPii(
  ...
  try {
    await registerFormalizationDisbursement(application.formalization.id, protectedReference, {
      actorType: 'OPERATOR',

      actorId: session.user.id,

      reason: 'Liberação financeira confirmada pelo operador.',
    });
  } catch (error) {
    if (
      error instanceof InvalidFormalizationStatusTransitionError ||
      ...
      return {
        error: 'O status da formalização mudou e a liberação não pôde ser registrada.',
      };
    }
    ...
    return {
      error: 'Não foi possível registrar a liberação.',
    };
  }

  revalidateFormalizationPaths(parsed.data.protocol);

  redirect(`/admin/solicitacoes/${encodeURIComponent(parsed.data.protocol)}/formalizacao`);
}

function revalidateFormalizationPaths(protocol: string) {
  revalidatePath('/admin/solicitacoes');

  revalidatePath(`/admin/solicitacoes/${protocol}`);

  revalidatePath(`/admin/solicitacoes/${protocol}/formalizacao`);

  revalidatePath(`/solicitacao/${protocol}/formalizacao`);
}
